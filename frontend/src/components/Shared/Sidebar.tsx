...
export const Sidebar: React.FC<SidebarProps> = ({ activeItem = 'Incidents' }) => {
  const menuItems = [
    ...
    { name: 'Incidents', icon: AlertOctagon, active: true },
    { name: 'Deployments', icon: GitBranch },
    { name: 'Threat Hunting', icon: SearchCode },
  ];
  ...
  return (
    <aside className="w-64 border-r border-darkBorder bg-[#090D16] flex flex-col justify-between p-4 select-none min-h-[calc(100vh-4rem)]">
      ...
      <div className="space-y-1 pt-4 border-t border-darkBorder/40">
        <button className="w-full flex items-center space-x-3 px-4 py-2 text-sm text-slate-400 hover:text-slate-200 transition-colors">
          <HelpCircle className="w-4 h-4 text-slate-500" />
          <span>Help Center</span>
        </button>
        <button className="w-full flex items-center space-x-3 px-4 py-2 text-sm text-slate-400 hover:text-rose-400 transition-colors">
